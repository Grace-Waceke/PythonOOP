...
class Account:
    ...
    def transfer(self,amount,instance_account):
        ...
        if amount>= self.account_balance:
            return f"There is insufficient amount in your account"
        if isinstance(instance_account,Account):
            self.loan_balance-=amount
            instance_account.loan_balance += amount
            return f" Dear customer, you have transfered {amount} to {instance_account} account with the name {instance_account.full_name} and your new balance is {self.account_balance}"
